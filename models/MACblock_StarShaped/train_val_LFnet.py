...
def conv_relu_dia(bottom, nout, ks=3, stride=1, pad=1,dilation=1,param=param):
    conv = L.Convolution(bottom, kernel_size=ks, stride=stride,
...
def cn_LF(split):
    ...
    n.LF_conv1, n.LF_relu1 = conv_relu(n.data, 32,ks=1, stride=9,pad=5,param=[dict(lr_mult=10, decay_mult=1), dict(lr_mult=20, decay_mult=0)])
    n.LF_conv2, n.LF_relu2 = conv_relu(n.data, 64, stride=9,pad=6,param=[dict(lr_mult=10, decay_mult=1), dict(lr_mult=20, decay_mult=0)])
    n.LF_conv3, n.LF_relu3 = conv_relu_dia(n.data, 64, pad=7,stride=9,dilation=2,param=[dict(lr_mult=10, decay_mult=1), dict(lr_mult=20, decay_mult=0)])
    n.LF_conv4, n.LF_relu4 = conv_relu_dia(n.data, 64, pad=8,stride=9,dilation=3,param=[dict(lr_mult=10, decay_mult=1), dict(lr_mult=20, decay_mult=0)])
    n.LF_conv5, n.LF_relu5 = conv_relu_dia(n.data, 64, pad=9,stride=9,dilation=4,param=[dict(lr_mult=10, decay_mult=1), dict(lr_mult=20, decay_mult=0)])
    
    bottom_layers = [n.LF_relu1, n.LF_relu2,n.LF_relu3,n.LF_relu4,n.LF_relu5]
    n.join = L.Concat(*bottom_layers)
    
    n.conv1, n.relu1 = conv_relu(n.join, 64,ks=1,pad=0,param=[dict(lr_mult=10, decay_mult=1), dict(lr_mult=20, decay_mult=0)])
    
    n.conv2, n.relu2 = conv_relu(n.relu1, 64,param=[dict(lr_mult=10, decay_mult=1), dict(lr_mult=20, decay_mult=0)])
...
def make_net():
    with open('../models/MACblock_StarShaped/train.prototxt', 'w') as f:
        f.write(str(cn_LF('train')))

    with open('../models/MACblock_StarShaped/val.prototxt', 'w') as f:
